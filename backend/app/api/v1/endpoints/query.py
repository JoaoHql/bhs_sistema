...
class SalesProjectionMatrixRequest(SalesProjectionRequest):
    pass


class SalesProjectionMatrixResponse(SalesProjectionWeeklyResponse):
    matrix_rows: list[dict] = Field(default_factory=list, alias="matrixRows")
...
@router.post("/sales-projection-matrix", response_model=SalesProjectionMatrixResponse)
async def sales_projection_matrix(
    request: SalesProjectionMatrixRequest,
    user: User = Depends(get_current_user),
    repo: ConfigRepositoryProtocol = Depends(get_repository),
    query_repo: QueryRepositoryProtocol = Depends(get_query_repository),
    redis: RedisService = Depends(get_redis_service),
    audit: AuditService = Depends(get_audit_service),
    x_tenant_slug: str | None = Header(default=None, alias="X-Tenant-Slug"),
) -> SalesProjectionMatrixResponse:
    client_slug = resolve_tenant_for_request(user, x_tenant_slug)
    client = await resolve_request_client(user, client_slug, repo)

    if request.screen_id not in ("projecao-vendas", "projecao-semanal"):
        from app.core.errors import NotFoundError
        raise NotFoundError("Tela nao encontrada.")

    cache_key = _query_cache_key(client_slug, "sales-projection-matrix", {
        "v": 5,
        "month": request.month,
        "company": request.company,
        "quantityGrowthPct": request.quantity_growth_pct,
    ...
    cached = await redis.get_json(cache_key)
    if cached is not None:
        return SalesProjectionMatrixResponse(**cached)
    ...
    try:
        ...
        result = await query_repo.fetch_sales_projection_matrix(
            schema_name=schema_name,
            month=request.month,
            company=request.company.strip() if request.company else None,
            quantity_growth_pct=request.quantity_growth_pct,
        ...
        audit.log_action_later(
            ...
            resource_id=request.screen_id,
            status="success",
            metadata={"template": "sales-projection-matrix", "month": result["month"]},
        )
        response = SalesProjectionMatrixResponse(screenId=request.screen_id, clientSlug=client_slug, **result)
        await redis.set_json(cache_key, response.model_dump(by_alias=True), ttl_seconds=CACHE_TTL_24H)
        return response
    except ApiError as exc:
        await audit.log_action(
            ...
            resource_id=request.screen_id,
            status="failed",
            metadata={"template": "sales-projection-matrix", "error": exc.message},
        )
        raise exc
