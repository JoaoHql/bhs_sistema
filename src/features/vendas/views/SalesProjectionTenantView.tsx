...
export const SalesProjectionTenantView: React.FC<Props> = ({ screenId }) => {
  ...
  const { data, isLoading, error } = useTenantData<SalesProjectionResponse>({
    ...
    params: { month, company, ...scenario }, refreshVersion: screenRefreshVersion,
    loader: () => queryApi.salesProjection({ screenId, month, company, ...scenario }),
  });
  ...
  const onScenarioChange = useCallback((field: keyof SalesProjectionScenario, value: number) => setScenario((current) => ({ ...current, [field]: value })), []);
